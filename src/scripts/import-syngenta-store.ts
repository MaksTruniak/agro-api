...
async function collectProducts(): Promise<ParsedProduct[]> {
  ...
  for (const category of CATEGORIES) {
    ...
    $('.catalog-products__item').each((_, card) => {
      ...
      const activeIngredient = clean($(card).find('.product__text a').first().text())
...
async function saveActiveIngredient(productId: string, activeIngredient: string) {
...
async function importProduct(product: ParsedProduct, manufacturerId: string) {
  ...
  await saveActiveIngredient(savedProduct.id, product.activeIngredient)
